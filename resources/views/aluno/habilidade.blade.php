                            ...
                            <div class="tab-pane fade show active" id="aba1" role="tabpanel">
                                <div class="mb-3">
                                    ...
                                    <select id="aluno_select" name="aluno_id" class="form-select" required>
                                        <option value="" disabled {{ old('aluno_id') ? '' : 'selected' }}>-- selecione
                                            --</option>
                                        ...
                                        @foreach ($alunosOrdenados as $a)
                                            @php
                                                $idadeExib =
                                                    $a->idade ??
                                                    (!empty($a->data_nascimento)
                                                        ? \Carbon\Carbon::parse($a->data_nascimento)->age
                                                        : null);
                                                $selected =
                                                    (string) old('aluno_id') === (string) $a->id ? 'selected' : '';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function normalizeAnalisePayload(payload) {
                ...
                if (payload.tecnicos || payload.fisicos || payload.composicao || payload.saude) {
                    Object.assign(fromGroups, payload.tecnicos || {}, payload.fisicos || {}, payload.composicao ||
                        {}, payload.saude || {});
                }
            ...
            function preencherAnalise(payload) {
                ...
                tecnicos.forEach(campo => {
                    ...
                    if (!el) return;
                    el.value = (normalized[campo] !== undefined && normalized[campo] !== null) ? normalized[
                        campo] : '';
                    el.readOnly = false;
                    el.disabled = false;
                });
