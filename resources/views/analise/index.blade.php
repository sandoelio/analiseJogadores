...
@push('styles')
    <style>
        #graficoFisico { min-height: 290px; }
        ...
        .field-wrapper, .chart-wrapper { position: relative; }
        ...
        .timeline { position: relative; padding-left: 36px; margin-top: 8px; margin-bottom: 8px; }
        .timeline::before {
            content: "";
            position: absolute;
        ...
        .marker-created { background: #28a745; }
        .marker-updated { background: #0d6efd; }
        .marker-other { background: #6c757d; }
        .timeline-marker i { font-style: normal; font-size: 12px; }

        .timeline-content { margin-left: 44px; width: 100%; }
        .timeline-header { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
        .timeline-time { font-weight: 700; }
        .timeline-user { color: #6c757d; font-size: 0.9rem; }
        .timeline-summary { color: #6c757d; font-size: 0.9rem; margin-top: 6px; }
        .timeline-actions { margin-left: 12px; flex-shrink: 0; }
        ...
        #modalTimeline .modal-header, #modalEventoDetalhes .modal-header {
            background: transparent;
        ...
        @media (max-width: 576px) {
            ...
            .action-buttons { gap: 0.75rem; justify-content: center; }
            .action-buttons .btn { margin-bottom: 0.5rem; }
            .action-buttons.stack-mobile .btn { width: 100%; }
        }

        @media (min-width: 768px) {
            html, body { overflow-x: hidden; }
            .back-logo { margin-top: 1rem; margin-bottom: 1.0rem; }
            #selecao-container { --bs-gutter-y: .5rem; margin-bottom: 0.1rem !important; }
            #selecao-container .field-wrapper { width: 100% !important; max-width: 600px; }
            .chart-wrapper { max-width: 600px; margin: 0.5rem auto 2rem; }
        }
    </style>
@endpush
...
@section('content')
    @php
        $instituicaoId = session('aluno_instituicao_id')
            ?? (auth()->check() ? (auth()->user()->instituicao_id ?? null) : null);
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function carregarAlunosDaInstituicao(instId) {
            ...
            if (window.INSTITUICAO_ID) {
                carregarAlunosDaInstituicao(window.INSTITUICAO_ID);
            } else {
                overlayAluno.classList.add('d-none');
                selectAluno.innerHTML = '<option selected disabled>Nenhuma instituição vinculada ao usuário</option>';
            }
            ...
            if (selectAluno) {
                selectAluno.addEventListener('change', () => {
                    ...
                    fetch(tplShow.replace('MATRICULA_ID', selectAluno.value), { credentials: 'same-origin' })
                        .then(r => {
                            if (!r.ok) throw new Error('HTTP ' + r.status);
                            return r.json();
                        })
                        .then(data => {
                            ...
                            chartInstance = new Chart(canvas.getContext('2d'), {
                                ...
                                data: {
                                    labels: data.labels,
                                    datasets: [
                                        { label: 'Anterior', data: data.anterior, backgroundColor: 'rgba(255,159,64,0.7)', borderRadius: 4, maxBarThickness: 50 },
                                        { label: 'Atual', data: data.atual, backgroundColor: 'rgba(75,192,192,0.8)', borderRadius: 4, maxBarThickness: 50 },
                                    ]
                                },
                                options: {
                                    responsive: true,
                                    ...
                                    scales: {
                                        x: { ticks: { autoSkip: true, maxRotation: 45, font: { size: 12 } } },
                                        y: { beginAtZero: true, suggestedMax: 20, ticks: { stepSize: 10, font: { size: 12 } } }
                                    },
                                    layout: { padding: { top: 10, bottom: 10 } },
                                    plugins: {
                                        datalabels: { anchor: 'end', align: 'end', color: '#444', offset: 4, formatter: v => v },
                                        legend: { position: 'top' }
                                    }
                                },
                        ...
                        if (ctxFisicoEl) {
                            ...
                            graficoFisicoInstance = new Chart(ctxFisico, {
                                ...
                                data: {
                                    ...
                                    datasets: [
                                        { label: 'Anterior', data: data.fisico.anterior, backgroundColor: 'rgba(255,159,64,0.7)', borderRadius: 4, maxBarThickness: 50 },
                                        { label: 'Atual', data: data.fisico.atual, backgroundColor: 'rgba(75,192,192,0.8)', borderRadius: 4, maxBarThickness: 50 },
                                    ]
                                },
                                options: {
                                    responsive: true,
                        ...
                        if (ctxClinicoEl) {
                            ...
                            graficoClinicoInstance = new Chart(ctxClinico, {
                                ...
                                data: {
                                    ...
                                    datasets: [
                                        { label: 'Anterior', data: data.clinico.anterior, backgroundColor: 'rgba(255,159,64,0.7)', borderRadius: 4, maxBarThickness: 50 },
                                        { label: 'Atual', data: data.clinico.atual, backgroundColor: 'rgba(75,192,192,0.8)', borderRadius: 4, maxBarThickness: 50 },
                                    ]
                                },
                                options: {
                                    responsive: true,
                ...
                fetch(url, { credentials: 'same-origin' })
                    ...
                    .then(json => {
                        ...
                        events.forEach(ev => {
                            ...
                            const monthLabel = dt.toLocaleString('pt-BR', { month: 'long', year: 'numeric' });
                            (grouped[monthLabel] = grouped[monthLabel] || []).push(ev);
                        });

                        let html = '';
                        Object.keys(grouped).sort((a, b) => {
                            const toKey = s => new Date(s.split(' de ').reverse().join('-') + '-01');
                            ...
                            grouped[monthLabel].forEach(ev => {
                                ...
                                const user = ev.changed_by ? `<span class="timeline-user"> — por ${escapeHtml(ev.changed_by)}</span>` : '';
                                const resumoBreve = ev.evento === 'analise_created' ? 'Atleta Atualizado' : (ev.evento === 'created' ? 'Atleta criado' : 'Evento');

                                const isCreated = (ev.evento === 'created') || (ev.evento === 'analise_created' && !(ev.dados && ev.dados.diff && Object.keys(ev.dados.diff).length));
                                const hasDiff = ev.dados && ev.dados.diff && typeof ev.dados.diff === 'object' && Object.keys(ev.dados.diff).length > 0;
                                const markerClass = isCreated ? 'marker-created' : (hasDiff ? 'marker-updated' : 'marker-other');
                        ...
                        if (container) {
                            container.querySelectorAll('.timeline-item').forEach(item => {
                                ...
                                if (btn) btn.addEventListener('click', () => window.verDetalhesEvento(id, evento));
                            });
                        }
                    })
                    .catch(() => {
                        if (overlay) overlay.classList.add('d-none');
                        if (container) container.innerHTML = '<div class="alert alert-danger">Erro ao carregar a linha do tempo.</div>';
                    });
            };
            ...
            window.verDetalhesEvento = function(id, evento) {
                ...
                fetch(url, { credentials: 'same-origin' })
                    ...
                    .then(json => {
                        ...
                        const hasDiff = json && json.dados && json.dados.diff && typeof json.dados.diff === 'object' && Object.keys(json.dados.diff).length > 0;
                        const effectiveEvento = (evento === 'analise_created' && hasDiff) ? 'updated' : evento;
                    ...
                    .catch(() => {
                        if (overlay) overlay.classList.add('d-none');
                        if (conteudo) conteudo.innerHTML = '<div class="alert alert-danger">Erro ao carregar detalhes do evento.</div>';
                        modalEl.show();
                    });
            };
            ...
            document.querySelectorAll('.modal').forEach(modal => {
                modal.addEventListener('hidden.bs.modal', () => {
                    if (graficoFisicoInstance) { graficoFisicoInstance.destroy(); graficoFisicoInstance = null; }
                    if (graficoClinicoInstance) { graficoClinicoInstance.destroy(); graficoClinicoInstance = null; }
                    const classificacao = document.getElementById('classificacaoLabel');
                    if (classificacao) { classificacao.textContent = ''; classificacao.classList.add('d-none'); }
                });
            });
        });
    </script>
